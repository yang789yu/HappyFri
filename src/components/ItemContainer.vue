<template>
    <section>
        <header class="top_tips">
            <span class="num_tip" v-if="fatherComponent === 'HomeIndex'">{{ level }}</span>
            <span class="num_tip" v-if="fatherComponent === 'ItemIndex'">题目{{ itemNum }}</span>
        </header>
        <div v-if="fatherComponent === 'HomeIndex'">
            <div class="home_logo item_container_style"></div>
            <router-link class="start button_style" to="Item"></router-link>
        </div>
        <div v-if="fatherComponent === 'ItemIndex'">
                ...
                <div class="item_list_container" v-if="itemDetail.length > 0">
                    ...
                    <ul>
                        <li v-for="(item, index) in itemDetail[itemNum - 1].topic_answer" :key="index" @click="choosed(index,item.topic_answer_id)" class="item_list">
                            <span class="option_style" v-bind:class="{'has_choosed':choosedIndex === index}">{{ choosedType(index) }}</span>
                            <span class="option_detail">{{ item.answer_name }}</span>                          
                        </li>
                    </ul>
                </div>
            </div>
            <span v-if="itemNum < itemDetail.length" @click="nextItem" class="next_item button_style"></span>
...
export default {
    ...
    created() {
        // 初始化信息
        if(this.fatherComponent === 'HomeIndex'){
            this.initializeData();
        }
    }
